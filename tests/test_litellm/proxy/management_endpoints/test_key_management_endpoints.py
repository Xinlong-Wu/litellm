...
async def test_list_keys_with_expand_user_includes_created_by_user():
    ...
    mock_user_owner.model_dump = MagicMock(
        return_value={
            "user_id": "user123",
...
async def test_update_key_with_router_settings(monkeypatch):
    ...
    update_request = UpdateKeyRequest(key="test-token-router", router_settings=router_settings_data)
...
async def test_rotate_master_key_model_data_valid_for_prisma(
    ...
    assert "created_at" not in model_data, "created_at should be excluded so Prisma @default(now()) applies"
    assert "updated_at" not in model_data, "updated_at should be excluded so Prisma @default(now()) applies"
...
def test_update_key_skips_org_check_when_no_throughput_fields_changed():
    ...
    data_with_org = UpdateKeyRequest(key="sk-test-key", organization_id="new-org")
...
def test_enforce_upperbound_fills_defaults_on_generate():
    ...
    original = litellm.upperbound_key_generate_params
    try:
        litellm.upperbound_key_generate_params = LiteLLM_UpperboundKeyGenerateParams(tpm_limit=1000, rpm_limit=100)
...
def test_enforce_upperbound_duration_over_limit():
    ...
    original = litellm.upperbound_key_generate_params
    try:
        litellm.upperbound_key_generate_params = LiteLLM_UpperboundKeyGenerateParams(duration="7d")
...
async def test_execute_virtual_key_regeneration_rejects_over_limit_duration():
    ...
    original = litellm.upperbound_key_generate_params
    try:
        litellm.upperbound_key_generate_params = LiteLLM_UpperboundKeyGenerateParams(duration="1h")
...
async def test_execute_virtual_key_regeneration_rejects_over_limit_max_budget():
    ...
    original = litellm.upperbound_key_generate_params
    try:
        litellm.upperbound_key_generate_params = LiteLLM_UpperboundKeyGenerateParams(max_budget=10.0)
        existing_key = _make_regenerate_existing_key()
...
async def test_build_model_max_budget_usage_reads_current_cache_window():
    ...
    mock_user_api_key_cache.async_get_cache.assert_awaited_once_with(key="virtual_key_spend:some-hash:gpt-4o:30d")
...
async def test_cli_session_token_personal_key_with_budget_blocked():
    ...
    with pytest.raises(HTTPException) as exc_info:
        await _common_key_generation_helper(
            ...
            user_api_key_dict=caller,
            litellm_changed_by=None,
            team_table=None,  # no team in request = personal key
        )
    assert exc_info.value.status_code == 400
...
async def test_cli_session_token_personal_key_without_budget_allowed():
    ...
    with patch(
        ...
        result = await _common_key_generation_helper(
            data=GenerateKeyRequest(max_budget=None),
            user_api_key_dict=caller,
            litellm_changed_by=None,
