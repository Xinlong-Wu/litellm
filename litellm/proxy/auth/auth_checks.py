...
async def common_checks(
    ...
    if not skip_budget_checks:
        ...
        ## 4.3 check per-model-group budgets (user-level + team-member-level)
...
def _normalize_models(model: Optional[Union[str, List[str]]]) -> List[str]:
    if model is None:
        return []
    if isinstance(model, list):
...
async def get_budgeted_groups_for_model(
    ...
    """
    matched: List[str] = []
    for group_id in model_group_max_budget.keys():
